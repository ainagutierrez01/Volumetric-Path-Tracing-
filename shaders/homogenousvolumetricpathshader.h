#ifndef HOMOGENOUSVOLUMETRICPATHSHADER_H
#define HOMOGENOUSVOLUMETRICPATHSHADER_H

#include "shader.h"

class Homogenous : public Shader
{
public:
    Homogenous();
    Homogenous(Vector3D hitColor, Vector3D bgColor_);
    ...
    Vector3D computeInscattering(const Ray& r, double t_max, const std::vector<Shape*>& objList, const std::vector<LightSource*>& lsList, int depth) const;
