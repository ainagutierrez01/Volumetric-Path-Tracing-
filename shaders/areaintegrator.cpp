...
AreaIntegrator::AreaIntegrator(Vector3D Color_, Vector3D bgColor_):
    ...
    if (its.shape->getMaterial().hasDiffuseOrGlossy()) {
        ...
        for (int lightId = 0; lightId < lsList.size(); lightId++) {
            ...
            for (int j = 0; j < numSamples; j++) {
                ...
                Vector3D wi = (p - its.itsPoint).normalized();
                double distance = (p - its.itsPoint).length();
                double dSquared = distance * distance;

                Ray testRay(its.itsPoint, wi);
                testRay.maxT = distance - 1e-4;
                bool is_in_shadow = Utils::hasIntersection(testRay, objList);

                if (!is_in_shadow) {
                    Vector3D radiance = lsList.at(lightId)->getIntensity();

                    Vector3D fr = its.shape->getMaterial().getReflectance(n, -wi, wo);

                    Vector3D ny = lsList.at(lightId)->getNormal().normalized();
                    double g = (dot(n, wi) * dot(-wi, ny)) / dSquared;

                    direct += (radiance * fr * g) / (pdf * numSamples);
                }
            }
        }
        Color = direct + (ambient * diffuse_coefficient);
    }
